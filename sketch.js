...
function setup() 
{
  ...
  document.addEventListener("click", (e) => {
    ...
    const shapeButtonsEl = document.querySelector("#shape-buttons");
    const textButtonsEl = document.querySelector("#dynamic-buttons");
    const restartBtn = document.querySelector("#btn-restart");
    const questionnaire = document.querySelector("#questionnaire-overlay");

    // 如果点击发生在任何“交互区域”，就什么都不做
    if (
      canvasEl?.contains(e.target) ||
      shapeButtonsEl?.contains(e.target) ||
      textButtonsEl?.contains(e.target) ||
      restartBtn?.contains(e.target) ||
      questionnaire?.contains(e.target)
    ) {
      return;
    }
...
buttons.forEach((btn, index) => {
  btn.addEventListener("click", () => {
    const textarea = pages[index].querySelector("textarea");
    const value = textarea.value.trim();

    questionnaireAnswers.push(value);
...
function processQuestionnaireAnswers() {
